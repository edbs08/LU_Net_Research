...
class Settings(object):
    ...
    def __init__(self, required_args=["config", "dataset", "gpu"]):
        ...
        if "gpu" in required_args and options.gpu:
            ...
            print('Please specify --gpu to select a GPU. You can run "gpustat" in order to see which GPU is available '
                  'on the machine.')
            exit(0)
        ...
        # SETTING VALUES
        if "config" in required_args:
            self.LIDAR_DATASET = config["DATA"]["lidar_dataset"]
            self.SEMKITTI_CFG = config["DATA"]["semkitti_cfg"]
            self.TFRECORD_TRAIN = eval(config["DATA"]["tfrecord_train"])
